...
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (idProject === '' || idProduct === '') {
            console.log('Parameters: ', idProject, idProduct);
            ...
            if (res.rowCount === 0) {
                ...
                const newProducts = deleteProductFromProject(Number(idProduct), productsInProject);

                if (newProducts === productsInProject) {
                    throw new Error('Product not found in project.');
                } else {
                    let res = await pool.query(`UPDATE projects SET products = $1 WHERE id = $2 RETURNING *;`, [JSON.stringify(newProducts), idProject]);
                    if (!res) {
